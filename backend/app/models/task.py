...
class Task(Base, UUIDMixin, TimestampMixin):
    ...
    # Workspace the task lives in (personal or team). All task access is
    # authorised through workspace membership.
    workspace_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("workspaces.id", ondelete="CASCADE"),
        nullable=False,
    )
    # Optional project grouping within the workspace. Deleting a project just
    # unlinks its tasks (SET NULL), it does not delete them.
